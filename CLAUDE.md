# AI-SOP-Protocol (ASP) v5.2.0 — 行為憲法
...
四層強制力（L1 SessionStart → L1.5 PreToolUse → L2 Dynamic Deny → L3 Skill Gates → L4 Subagent QA）逐層機制細節見 [docs/claude-md-reference.md](docs/claude-md-reference.md#強制力架構四層機制)。
...
- 多 session 並行同一 repo → **每 session 開專屬 git worktree**（issue→worktree→branch→PR，主樹留人類；ADR-027 advisory）
...
常用 `make` 指令表與「ASP slash 指令三形式（`/asp <意圖>` 治理工作流 vs `/asp:<動作>` 單步快捷）」命名慣例見 [docs/claude-md-reference.md](docs/claude-md-reference.md#常用指令)。完整指令：`make help`｜入門文件：`docs/where-to-start.md`、`docs/quickstart.md`
...
| Domain docs | `CONTEXT.md` + `GLOSSARY.md` at root + `docs/adr/` — see `docs/agents/domain.md` |

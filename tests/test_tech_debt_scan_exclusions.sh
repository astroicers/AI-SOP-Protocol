...
COUNT=$(grep -c "DUE:" <<<"$HITS" || true); COUNT=${COUNT:-0}
[ "$COUNT" -eq 1 ] && pass "exactly 1 hit (real marker only, example excluded)" \
    || fail "expected 1 hit, got $COUNT: $HITS"
grep -q "real.sh" <<<"$HITS" && pass "the surviving hit is the real marker" \
...
if grep -qE "global_core.md|asp-compiled-profile" <<<"$REPO_HITS"; then

<!-- Last Updated: 2026-06-11 | Status: Active | Audience: All users -->
# AI-SOP-Protocol — 系統架構總覽

> 本文件是 ASP v5 的 **架構入口**。詳細設計散落在 ADR、SPEC 與 SDS 中，本文負責把它們連成一張地圖。
>
> **適用版本**：v5.0.0 候選（2026-06-11，asp/v5-slimming）
...
設計脈絡與哲學詳見 [`v4-architecture-sds.md`](archive/v4-refactor/v4-architecture-sds.md) §1-2 與 [`adr/ADR-002-asp-v4-security-threat-model.md`](adr/ADR-002-asp-v4-security-threat-model.md)。

---

## 2. 三層架構 + Compile 層（v5，ADR-016）

```mermaid
flowchart TB
    subgraph User["User-level（~/.claude/）— 一次安裝、跨專案共用"]
        UCM["CLAUDE.md<br/>(鐵則 + loose/standard/autonomous)"]
        SKILLS["skills/asp/<br/>(15 個 skill)"]
        ASP["asp/profiles (12)<br/>asp/hooks<br/>asp/templates<br/>asp/levels (3)<br/>asp/config<br/>asp/scripts"]
    end

    subgraph Project["Project-level（每個專案）— 輕量設定"]
        PCM["CLAUDE.md<br/>(精簡版)"]
        PROFILE[".ai_profile<br/>(type/level/mode)"]
        SETTINGS[".claude/settings.json<br/>(hook 指向 user-level)"]
        DOCS["docs/adr/<br/>docs/specs/"]
    end

    subgraph Compile["Compile 層（v5 ADR-016）— 依賴解析移到編譯期"]
        MAP["profile-map.yaml<br/>(single source of truth)"]
        COMPILER["asp-compile.sh<br/>(選集→拓撲展開→衝突裁決)"]
        ARTIFACT[".asp-compiled-profile.md<br/>(gitignored 載入 artifact)"]
    end

    subgraph Runtime["Runtime — Session 啟動時"]
        AUDIT["session-audit.sh<br/>(+asp_metric 遙測, ADR-018)"]
        BRIEFING[".asp-session-briefing.json"]
        DENY["dynamic deny list"]
        HITS["~/.claude/asp/metrics/<br/>rule-hits.jsonl"]
    end

    PROFILE -->|讀取| COMPILER
    MAP -->|映射規則| COMPILER
    COMPILER -->|產出| ARTIFACT
    AUDIT -->|--check mtime 重編| COMPILER
    ARTIFACT -.->|AI 直接讀取（散文 fallback）| PCM
    PROFILE -->|讀取| AUDIT
    SETTINGS -->|觸發| AUDIT
    AUDIT -->|寫入| BRIEFING
    AUDIT -->|注入| DENY
...
| 路徑 | 內容 | 對應文件 |
|------|------|---------|
| `~/.claude/CLAUDE.md` | 鐵則 + 三級制等級表（v5） | [專案 CLAUDE.md 註腳](../CLAUDE.md) |
| `~/.claude/skills/asp/` | 15 個 skill（`asp-plan`、`asp-ship`、`asp-gate`、`asp-context` 等；3 個凍結至 experimental/） | [SKILL.md router](../.claude/skills/asp/SKILL.md) |
| `~/.claude/asp/profiles/` | 12 個 profile（`global_core`、`system_dev`、`loose_mode` 等；v5 ADR-014/017） | profile 集合 |
...
| `~/.claude/asp/levels/` | `loose/standard/autonomous.yaml` 成熟度定義（v5 三級制） | [`level0-spike-mode.md`](level0-spike-mode.md) |
| `~/.claude/asp/scripts/` | asp-compile / asp-metrics / level-resolve / rule-stats / orchestrator/（v5 確定性腳本層） | ADR-013/015/016/018 |
| `~/.claude/scripts/asp-sync.sh` | 從 repo 同步到 user-level | [`project-structure.md`](project-structure.md) |

**部署設計理由**（D-004, 2026-05-04）：N 個專案各裝一份 ASP 是 v3.7 的痛點；user-level centralization 用 Claude Code 內建 skill 載入機制達成「改一次全吃到」，沒有新攻擊面。詳見 [`v4-decision-log.md`](archive/v4-refactor/v4-decision-log.md) D-004。
...
## 5. 核心子系統（Subsystem Map）— v5 三分區（ADR-017）

```mermaid
flowchart LR
    subgraph CoreZone["🟢 Core（daily-driver，預設安裝）"]
        subgraph A["A. Constitution<br/>(鐵則 + 三級制)"]
            CLAUDE[CLAUDE.md]
            LEVELS["loose/standard/autonomous.yaml"]
        end
        subgraph B["B. Skills (15 個)"]
        ...
        subgraph C["C. Profiles (12 個) + Compile"]
            CORE[global_core]
            SYS[system_dev]
            TO["task_orchestrator (300 行)<br/>+ orchestrator 確定性腳本"]
            COMPILE["asp-compile.sh<br/>profile-map.yaml"]
        end
        subgraph E["E. Security + 規則遙測"]
            SEMGREP[".semgrep/asp-security.yml"]
            BYPASS[".asp-bypass-log.ndjson"]
            RULEHITS["rule-registry.yaml<br/>rule-hits.jsonl (ADR-018)"]
        end
    end

    subgraph ExpZone["🧊 Experimental（凍結，不安裝）"]
        MA["multi-agent worktree<br/>8 腳本 + 10 角色 + 3 skills<br/>+ orchestrator_multi_agent"]
    end

    subgraph ShowZone["🎓 Showcase（--with-showcase 裝回）"]
        D["Telemetry<br/>collect/report/prune"]
        RAG["RAG<br/>build_index/search"]
        F["AI Performance<br/>trust-tier/monthly-review"]
    end

    A -->|定義| C
    C -->|被 skill 引用| B
    B -->|gate-log / bypass 記錄| E
    C -.->|解凍條件：單 session<br/>無法完成的實際案例| MA
```

### A. Constitution（憲法層）

- **檔案**：`CLAUDE.md`、`loose.yaml`/`standard.yaml`/`autonomous.yaml`（v5 三級制，ADR-014）、`.ai_profile`
- **責任**：定義鐵則、等級、profile 載入規則
- **更新頻率**：低（架構變動才動）
- **完整文件**：[`v4-architecture-sds.md`](archive/v4-refactor/v4-architecture-sds.md) §3

### B. Skills（行為路由層）

- **檔案**：`.claude/skills/asp/asp-*.md`（15 個；asp-dispatch/team-pick/handoff 已凍結至 experimental/，ADR-017）
...
- **檔案**：`~/.claude/asp/profiles/*.md`（12 個，v5 ADR-014/017）
...
| 決策 | 簡述 | 文件 |
|------|------|------|
| **D-001** | Multi-agent 改用 `/clear` + scratchpad，不用 context 全量傳遞 | [`v4-decision-log.md`](archive/v4-refactor/v4-decision-log.md) D-001 |
| **D-002** | 安全違規規則改用 Semgrep ruleset（hard mechanism） | [`v4-decision-log.md`](archive/v4-refactor/v4-decision-log.md) D-002 |
| **D-003** | `auto_fix_loop` 補抓 false positive + adversarial evasion | [`v4-decision-log.md`](archive/v4-refactor/v4-decision-log.md) D-003 |
| **D-004** | User-level centralization（不引入「中央 ASP agent」） | [`v4-decision-log.md`](archive/v4-refactor/v4-decision-log.md) D-004 |
...
完整 decision log：[`v4-decision-log.md`](archive/v4-refactor/v4-decision-log.md)。
...
| 想理解設計哲學 | [`v4-architecture-sds.md`](archive/v4-refactor/v4-architecture-sds.md) §1-2 |

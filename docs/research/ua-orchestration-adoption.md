...
> 配套決策：[ADR-008](../adr/ADR-008-adopt-ua-orchestration-patterns-minimal.md)（`Draft`）。本報告為評估交付物：完整對照表、自動化摩擦評估、採納清單、diff 草案、開放問題。
> **狀態 `Draft`：所有 diff 草案待人類核准 ADR-008 升 `Accepted` 後方可實作。本報告不含已落地的 production code。**
...
| ADR 未定案禁止實作 | 本報告 + ADR-008 維持 `Draft`，所有 diff 為草案，待人類核准。 |
...
| # | 問題 | 決議 | 依據 |
|---|---|---|---|
| 1 | 單一 ADR vs 拆分 | **併一個 ADR-008**（實作層仍各拆 commit/PR） | ADR-002/006/007 先例 + ADR-006:67 反碎片化原則 |
...
**仍須人類拍板**：ADR-008 是否升 `Accepted`（鐵則：Draft 禁止實作）。

...
- 真正值得做的只有 4 項——B/C0 修漏洞、A 精簡、C 收緊（⚠️ **經 5-lens 審查更正：非「淨減摩擦」**，見 §10）：
  ...
  4. **Pattern C**（trivial-tier binary gate）— ⚠️ **doc-only / 待 code-enforced**；是**收緊**現有無條件 auto-merge（非減摩擦），現況無 parser 消費
...
| **C** binary gate | **收緊** trivial auto-merge（TIER_3/2 本已無條件 `auto_merge:true`）；doc-only 無 parser | **中性偏負**（新增阻擋條件） | 低 | 對齊 trust-tier |
...
| **④** | Pattern C binary gate | Low（doc-only） | Low | Med | 中性偏負 | **doc-only / 待 code-enforced** |
...
# ⚠️ 經 5-lens 審查再修正三點：
#   (a) fail-closed：git diff 失敗（CI shallow clone / base 不可達）時須【擋】而非 `|| true` 放行，否則 crypto fail-open。
#   (b) regex 補 false-negative：補 aes/rsa/hmac/kdf/secretbox/encryptor/cryptobox + secrets 目錄。
#       C0 定位為 best-effort 偵測（非完整保證；改名可規避，僅為防線之一）。
#   (c) emit_escalation 目前無 severity 欄（converge.sh:77）→ 'P0' 須先擴充簽名或寫進 details，勿假設可寫。
# set -e 安全：用 `if ! VAR=$(...)` 捕捉 git diff 失敗；裸 `VAR=$(...)` 會把 git 非零傳播 → set -e 中止腳本。
if ! DIFF_FILES=$(git -C "$ASP_AUDIT_ROOT" diff --name-only "$BASE_BRANCH..$TASK_BRANCH" 2>/dev/null); then
    emit_escalation "$tid" "crypto_gate_diff_failed" "base 不可達，無法確認 crypto → fail-closed 擋下"
    CRYPTO_SKIPPED=1; continue                                # fail-closed：無法判定即擋
fi
CRYPTO=$(printf '%s\n' "$DIFF_FILES" | grep -Ei \
...
# ⚠️ 第二輪 review 修正：前導邊界改 (^|[^a-z]) 讓 ^ 真正當錨點（首版 (_|-|\.|/|^) 對根層裸檔名 aes.go/secretbox.go 全漏）；
#    完整動詞 encrypt/decrypt 允許後綴、縮寫詞 kms/aes… 要求後接非字母（排 kmstore FP）。實測 14/15 命中、0 FP；
#    仍漏駝峰連寫 AESEncrypt.go → C0 本質 best-effort，完整保證靠 crypto/ 目錄 + cross-vendor 4/4 + human。
if [ -n "$CRYPTO" ]; then
    emit_escalation "$tid" "crypto_path_touched" "$CRYPTO"   # 強制 HITL（severity 見上註 (c)）
    emit_telemetry "multi_agent.fail" "$tid" "crypto_hitl"
    CRYPTO_SKIPPED=1
    continue                                                 # 不 merge，永不 auto-repair bytes（迴圈內）
fi
```
> **不**實作 C1（已有 `scope-guard.sh` PreToolUse）、C3（已有 `auto_fix_loop`）、C5（已有 `asp-ship` Step 9）。避免重複層 = 避免 over-engineering。
> **失敗語意已決：`continue`**（per-task 跳過、其他續跑，符合 partial-success）。⚠️ 其他 task 全成功時 loop 會 fall-through `exit 0` 遮蔽 P0 → for 迴圈結束後加（**set -e 安全形式**）：`if [ "${CRYPTO_SKIPPED:-}" = "1" ]; then exit 9; fi`（dedicated `crypto_hitl_pending`；**不可**用 `[ … ] && exit 9` 作末句——CRYPTO_SKIPPED≠1 時回 rc=1 會被 set -e 變成 exit 1 打掛正常 converge）。
...
### 5.4 Pattern C：`trust-tier.yaml` binary gate（⚠️ doc-only / 待 code-enforced）

> **5-lens 審查更正**：(1) TIER_3/TIER_2 現況已 `auto_merge: true` 無條件 → 本 gate 是**收緊（增阻擋）非減摩擦**；(2) repo 無 parser 消費此區塊、`touches_sensitive_path` 在 code 中**不存在**、`ai_classification` 是 AI 自填無強制 → 「high-stakes 永不繞過」目前**無執行力**；(3) 升 Accepted 為正式採納前，trivial 分類與 sensitive-path early-return 必須**確定性 code 強制**（production-ops §4 + 與 C0 同款 path regex），否則僅 doc-only（同 Pattern 8）。
...
| high-stakes = cross-vendor 4/4 + human | Pattern C binary gate **僅 trivial tier**，且現為 **doc-only**——`touches_sensitive_path` early-return + 確定性 trivial 分類須 **code-enforced** 後才具保護力（現況無 parser，見 §5.4）。high-stakes 的真實 backstop 是既有 live 的 cross-vendor 4/4 + human（production-ops 鐵則 6/7、`asp-external-review`），未被本提案觸動。 |
| 不違反精簡/分級理念 | 明確拒絕 Pattern 3 C1–C6；**相對選項 A 不重做 C1/C3/C5 三層**（B/C0 為新增 fail-closed 卡點、C 為收緊——非「淨減摩擦」，見 §10）。 |
...
| # | 問題 | 決議 | 依據 |
|---|---|---|---|
| 1 | 單一 ADR vs 拆分 | **併一個 ADR-010**（實作層仍各拆 commit/PR；升 Accepted 時 C 降 doc-only 不同級） | 同源自 UA 評估、同治理面，便於一次 review（**非**因相依——四項低耦合；不採 ADR-006:67「有依賴者不可分」，前提相反） |
...
| converge C0 (3) | 插入點須在 `mktemp` 前否則 `continue` 洩漏暫存檔；`continue` 後 loop fall-through `exit 0` 遮蔽 P0 → 插入點前移 + `CRYPTO_SKIPPED`→`exit 9`；變數與 `emit_*` 簽名驗證正確。⚠️ **regex 經第二輪 review 實測修正**：首版 corrected（`(_\|-\|\.\|/\|^)` 前導）對**根層裸檔名**（aes/secretbox/keystore/hmac/kdf/cryptobox）仍全 false-negative、kmstore FP 未排除 → 已換 §5.2 第二版（前導 `(^\|[^a-z])`，實測 14/15 命中、0 FP，僅駝峰 AESEncrypt 漏 → C0 定位 best-effort）。 |
...
## 10. 5-lens 決策審查摘要（2026-06-08，獨立 reviewer workflow）

§9 是 diff 草案的「技術正確性」對抗驗證；本節是「決策層」獨立審查（該不該採納、論證品質、是否真不弱化 hard-constraint）。**5 個 lens 全 `accept_with_changes`（無 reject）**，must-fix 已併入 ADR-010 後果段與本報告 §5/§6：

| Lens | Verdict | 關鍵 must-fix |
|---|---|---|
| 採納決策正確性 | accept_with_changes | Pattern C framing 說反（**收緊非減摩擦**）；C 無執行路徑＝**dead spec**，應與 Pattern 8 同列 doc-only |
| 遺漏風險/盲點 | accept_with_changes | C0 regex **false-negative**（encryptor/aes/rsa/secretbox 漏接）；CI shallow clone 時 git diff 失敗 **fail-open**；C0 skip 後孤兒 worktree/branch + escalation 無 consumer |
| **hard-constraint 完整性** | accept_with_changes | `touches_sensitive_path` 在 code 中**不存在**、分類無強制 → high-stakes 可被誤標 trivial 繞過（須 code-enforced）；ADR 誤稱「Iron Rules A/B/C」（crypto/customer-facing 來源實為 production-ops 鐵則 6/7） |
| ADR 論證品質 | accept_with_changes | 「治理層數淨值不增」無法計數且自我反駁；「併一個 ADR」誤引 ADR-006:67（前提相反）；Success Metrics 兩列不可二元驗證 |
| SPEC 實作可行性 | accept_with_changes | `exit 9` 末句在 `set -eu` 下打掛每次正常 converge（須 `if…then`）；Pattern A/C 標 doc-only 但語氣像有強制力 |

**共識強項**：Pattern B 確為真實 latent bug 且修復正確；拒絕 C1/C3/C5 降規論證紮實；C0 analyze-only fail-safe 正確；cross-vendor 4/4+human 是 live 機制；對抗驗證誠實。

> 全部 must-fix 已反映於 ADR-010「技術債」段（標 `[must-fix]`）與本報告 §5/§6。**結論：Pattern B/C0 為可快速 Accept 的真價值；Pattern C 維持 doc-only 待 code-enforced。**

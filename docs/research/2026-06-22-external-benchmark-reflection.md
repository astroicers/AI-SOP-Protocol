...
1. **治理過載**——光是 ADR+SPEC 就 6,039 行治理決策文件，已超過全部功能碼（shell+python 共 4,141 行，不含測試）；20 個高度交叉引用的 ADR、為繞過 LLM 不穩而生的 2082 行編譯產物、帶 escape hatch 的機械閘、甚至有「量測自己有沒有過胖」的基礎建設。每遇一個 LLM 可靠性問題，反射動作是**加一層機械強制**，而非簡化需求。
...
- **連接邏輯散落在 20 個高度交叉引用的 ADR**。ADR-009 引用 7 個其他 ADR（佔總數 35%）；要懂一個決策得回溯 3–7 個前序。這條「思維路線」存在，但**讀者得自己在 20 份文件間拼湊**，沒有一份「定案設計總覽」把它串成一條線。
...
- **可行性（已查證）**：官方 plugin 機制**完整支援** ASP 賴以為生的元件——SessionStart / PreToolUse hooks（宣告於 `hooks/hooks.json`，腳本路徑用 `${CLAUDE_PLUGIN_ROOT}`，可 block tool call）、commands、skills、agents、MCP server，並透過 `.claude-plugin/marketplace.json` 經 `/plugin marketplace add` 一鍵安裝、依 commit/version 自動更新。唯一官方文件未明載的是「hook 動態寫 `settings.local.json`」，但 plugin hook 以非沙箱 shell 執行，技術上不受限。**故此建議是低風險、高信心，而非探索性提案。**（依據官方 docs：plugins / plugins-reference / plugin-marketplaces。）
...
- **數據**：
  - 治理文件量壓過功能碼：ADR+SPEC **6,039 行**、全 docs **19,254 行**，而功能碼（shell+python，不含測試）僅 **4,141 行**（比值約 4.6:1；以 `wc -l` 可重現）。
  - **20 個交叉耦合 ADR**，平均 2.4 個互引，ADR-009 引用 7 個其他 ADR（35%）。
  - `.asp-compiled-profile.md` **2082 行**編譯產物——**為繞過 LLM runtime 解析 profile 依賴不穩**而加的 build-time 編譯器（ADR-016）。
...
| **P1** | skill 拆小（§2）+ 接 Claude Code plugin marketplace（§4） | 中 | **低（已查證可行，見 §4）** | 可發現性 + 重用性 + 砍掉 sync 漂移測試 |
...
- **結構/數據**：`.asp-compiled-profile.md`(2082 行)、`.claude/skills/asp/asp-autopilot.md`(829)/`asp-gate.md`(419)/`asp-ship.md`(306)、`docs/adr/`(ADR-001..020)、`docs/specs/`(SPEC-001..013)、`.asp/scripts/install.sh`(29KB)、`tests/`(48 檔)、`tests/lib/common.sh`。

...
|------|------|
| **狀態** | `Accepted` |
| **日期** | 2026-06-11（Accepted: 2026-06-12） |
| **決策者** | ASP framework maintainers |

> ⬆️ 狀態歷程：`Draft` → `FIRM`（使用者 2026-06-11 asp-plan HITL 授權放行實作）→ `Accepted`（使用者 2026-06-12 經獨立 reality-checker review + 全部 findings 收尾後同意升級；非 AI 自行升級，符合 ADR 狀態變更鐵則）。殘留（tamper-evidence 邊界、選項 C 外部錨）為已知後續追蹤，不阻擋決策定案。
...
選 A 不選 C：C 與 local-only 決策張力大、流程沉重，超出本 ADR 範圍（列為後續獨立 ADR 追蹤）；但其外部錨思路保留為後續。
...
- **驗證器自我保護**（review HIGH finding 收尾）：chain 驗證器 `bypass-hash.sh` 納入 Iron Rule A `CRITICAL_FILE`（git-hash 完整性比對），防止「改 `verify()` 永遠回 `exit 0`」繞過整個 chain — 補上「看守者的看守者」缺口。
...
- [ ] 評估選項 C（chain tip 外部錨定）作為獨立 ADR — 需重新權衡 local-only vs tamper-proof。**Trigger 條件**：`mode: multi-agent` 啟用、或外部審計要求 tamper-proof 時優先評估（否則維持 backlog）
- [x] 升級 Iron Rule B 註解 + rule-registry desc 反映 hash-chain（已完成）
- [x] 驗證器 `bypass-hash.sh` 納入 Iron Rule A 保護（review HIGH 收尾，`test_iron_rule_a_coverage.sh`）
...
| 欄位 | 內容 |
|------|------|
| **POC 分支 / 測試結果** | 已實作並 merge（#28 + #29）；`tests/test_iron_rule_b_hashchain.sh` **15/15 PASS** + `test_iron_rule_a_coverage.sh`（驗證器自我保護）；`make test` 全綠（bash 44/44、pytest 16/16），含既有 7 個 HWM 截斷測試零回歸 |
| **驗證日期** | 2026-06-11 |
| **驗證者** | astroicers（使用者經 asp-plan HITL 授權升 FIRM 並放行實作） |
| **驗證摘要** | hash-chain 偵測等量替換 / chain 斷裂 / HWM 竄改獨立性 / 容錯降級繞過；正常 append 與空 log 無誤報；既有 7 個 HWM 截斷測試零回歸。獨立 reality-checker review 後修補威脅模型漏洞（驗證器納入 Iron Rule A）與 SPEC/實作 drift（#29），findings 全數收尾 |

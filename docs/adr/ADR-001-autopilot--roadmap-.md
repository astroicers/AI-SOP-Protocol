<!-- Last Updated: 2026-05-09 | Status: Accepted | Audience: autonomous users -->
...
- 取代：（無）
- 被取代：（部分）**[ADR-006](ADR-006-feature-audit-roadmap.md) Item 7 已於 v4.4 執行（SPEC-010，2026-06-11）**——`.asp/profiles/autopilot.md` 已刪除，完整執行規格整併入 `asp-autopilot` skill Part 2（唯一 canonical source）。C2 漂移（由 [ADR-012](ADR-012-define-operator-autopilot-interaction-trust-model.md) 揭露）就此結案。本 ADR 的 ROADMAP/任務佇列設計仍有效，僅承載檔案易位。
- 參考：
  ...
  - [ADR-012](ADR-012-define-operator-autopilot-interaction-trust-model.md) — operator↔autopilot 信任模型（provenance-scoped、授權隨架構影響縮放）；其 DP3 明示「人類手寫 ROADMAP 任務維持本 ADR 的現有機制不變」

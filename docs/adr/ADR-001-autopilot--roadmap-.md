...
- 取代：（無）
- 被取代：（部分）autopilot 的 **profile/skill 定位待 [ADR-006](ADR-006-feature-audit-roadmap.md) Item 7 執行**（移除 autopilot.md profile、邏輯收回 asp-autopilot skill）；在此之前 profile(582 行)＋skill 並存（C2 漂移，由 [ADR-012](ADR-012-define-operator-autopilot-interaction-trust-model.md) 揭露並點名）。
- 參考：

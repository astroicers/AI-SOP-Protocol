...
ASP v4.0 重構計劃（SPEC-003、`docs/archive/v4-refactor/v4-refactor-prompts.md` Prompt 4）原本預定實作一個 MCP server，提供至少 5 個 stateful tool（session log、bypass log query、trust tier lookup 等），讓 AI agent 在 session 中透過 MCP protocol 存取 ASP 狀態。
...
- `docs/archive/v4-refactor/v4-refactor-prompts.md` — Prompt 4 原始需求（已取消）

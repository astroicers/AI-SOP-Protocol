...
- [—] **共存冪等**：→ **降級為 documented known-limitation（#65 closed wontfix-until-needed，人類 2026-07-22 核准）**。深入設計後確認：安全 sentinel 須碰 L2 強制核心 hook——SessionStart 部分需在 `session-audit.sh` 讀 stdin `session_id`（阻塞/fail-open landmine，錯了會护欄失效）；**`ship-gate`（PreToolUse）以 `session_id` 為鍵會「第一個 commit 後全跳過」而破閘**，安全 dedup 需 per-tool-call id（現確認 stdin 僅 `{tool_name,tool_input.command,cwd}`，無此 id）。而 double-fire **傷害低**：僅 installer+plugin **共存**時發生、`SHIP-GATE` 高頻規則雙計對 rule-stats **無害**、僅稀見 session-start A-rule 微偏。故**不為 telemetry hygiene 碰 L2 核心**。重啟條件：A+B 共存成常態、或 plugin 全面取代 installer 時，以 per-call-id-aware 設計重議（見 #65）。

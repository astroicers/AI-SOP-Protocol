...
# Part 2 — 完整執行規格（canonical）

> **v4.4 起本 skill 為 autopilot 的唯一 canonical source（ADR-006 Item 7）**：
> 原 `.asp/profiles/autopilot.md` 已整併至本節並刪除。Part 1（上方）為操作視圖，
> 本 Part 為完整行為規格——兩者衝突時以本 Part 為準。
> 啟用前提（`.ai_profile` 的 `autopilot: enabled`、autonomous_dev/task_orchestrator
> 載入）由 CLAUDE.md 啟動程序負責（architectural delegation）；Context 管理
> （60%/75% 閾值）見 Part 1 Phase 3。
## 前置文件動態探測
...
FUNCTION autopilot_main():
  ...
  FOR task IN queue:
    ...
    IF result.success:
      ...
      // ─── Auto-PR：推送 feature branch + 建立 Draft PR ───
      branch_name = "asp/{task.id}-{slugify(task.title)}"  // 例：asp/TASK-001-fix-login-timeout
      EXECUTE("git checkout -b {branch_name} 2>/dev/null || git checkout {branch_name}")
      EXECUTE("git push origin {branch_name}")
      pr_body = "## {task.title}\n\n**Task:** {task.id}\n**Type:** {task.type}\n**SPEC:** {task.spec}\n\n### 變更摘要\n{result.summary}\n\n### 驗收條件\n{spec.done_when}\n\n---\n🤖 Auto-generated by ASP Autopilot"
      EXECUTE("gh pr create --draft --title '[{task.type}] {task.title}' --body '{pr_body}' --base main")
      LOG("🔀 Draft PR created for {task.id} on branch {branch_name}")
      EXECUTE("git checkout main 2>/dev/null || git checkout -")  // 回到主分支繼續下一個任務
...
## 與其他 Profile 的關係（v4.4 改寫：autopilot 已為 skill，非 profile）

```
asp-autopilot（skill，本檔）
  ├── 依賴 autonomous_dev.md profile（安全邊界 + auto_fix_loop）
  ├── 依賴 task_orchestrator.md profile（on_task_received() 入口 + 健康審計）
...
Autopilot 是最外層的調度邏輯，包裹其他 profile 的功能。

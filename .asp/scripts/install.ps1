...
function Test-Downgrade {
    # $true iff 兩者皆可解析為版本且 Source < Installed。非版本字串 → $false（不誤判方向）。
    param([string]$Installed, [string]$Source)
    $i = $null; $s = $null
    if (-not [version]::TryParse($Installed, [ref]$i)) { return $false }
    if (-not [version]::TryParse($Source,    [ref]$s)) { return $false }
    return ($s -lt $i)
}
...
$Downgrade = $false; $Upgrade = $false   # 版本方向（clone 後計算；提前宣告供完成訊息使用）
if ($IsUserUpgrade) {
    if (Test-Path (Join-Path $UserAsp 'VERSION')) {
        # 空/0-byte VERSION → Get-Content -Raw 回 $null；先存變數判空，避免 $null.Trim() 在 try 外崩潰
        $rawInstalled = Get-Content (Join-Path $UserAsp 'VERSION') -Raw -ErrorAction SilentlyContinue
        if ($rawInstalled) { $InstalledVersion = $rawInstalled.Trim() }
    }
...
try {
    ...
    # 先存變數判空：clone 來源缺 .asp\VERSION 時 Get-Content 回 $null，$null.Trim() 會擲例外被 catch 吞成「無法連接 GitHub」並繞過降級守門
    $rawNew = Get-Content (Join-Path $TmpDir '.asp\VERSION') -Raw -ErrorAction SilentlyContinue
    $NewVersion = if ($rawNew) { $rawNew.Trim() } else { 'unknown' }
    $NewCommit = (& git -C $TmpDir rev-parse --short HEAD 2>&1).Trim()
    Write-Host "  版本：v$NewVersion ($NewCommit)"

    # ─── 版本方向守門（防無聲降級；對應 install.sh 同款防護）───
    if ($IsUserUpgrade) {
        if (Test-Downgrade $InstalledVersion $NewVersion) {
            $Downgrade = $true
        } elseif (Test-Downgrade $NewVersion $InstalledVersion) {
            $Upgrade = $true
        }
    }
    if ($Downgrade) {
        Write-Warn "偵測到降級：已安裝 v$InstalledVersion 比來源 v$NewVersion 新（$NewCommit）"
        Write-Host '     來源可能落後（GitHub main 未更新 / ProtocolRepo 指向舊版）。'
        if ($env:ASP_ALLOW_DOWNGRADE -ne '1') {
            Write-Warn '已中止：預設不把 user-level 降級。確定要降級請設環境變數 ASP_ALLOW_DOWNGRADE=1 重跑。'
            exit 1
        }
        Write-Warn 'ASP_ALLOW_DOWNGRADE=1 — 強制降級'
    }
    ...
    if ($IsUserUpgrade) {
        if ($Downgrade) {
            Write-Success "User-level 降級完成（v$InstalledVersion → v$NewVersion）"
        } elseif ($Upgrade) {
            Write-Success "User-level 升級完成（v$InstalledVersion → v$NewVersion）"
        } else {
            Write-Success "User-level 同步完成（v$InstalledVersion → v$NewVersion）"
        }
    } else {
...
if ($Downgrade) {
    Write-Host "🎉 降級完成！（v$NewVersion @ $NewCommit）"
} elseif ($IsProjectUpgrade) {

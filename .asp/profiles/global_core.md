...
## 範疇與敏感資訊三層回應（v5 併入，ADR-014 D6）

> **設計原則**：護欄的目的是保護，不是阻礙。預設行為是「詢問與引導」，
> 只有敏感資訊才是「硬拒絕」。本節是 user-level 鐵則「敏感資訊保護」的執行細則。
...
## HITL 等級與暫停決策（v5 併入，ADR-014 D2 — 全域概念）
...
> 例外覆蓋：loose_mode 的 spike 豁免活動期間強制 `strict`（ADR-014 D1）；
> 需求變更 L2 以上必須暫停（見「需求變更回溯協議」共通規則）。

---

## 升級路徑（Escalation）（v5 併入，ADR-014 D4/D5）

agent 無法繼續時的分級升級。詳細決策樹（PAUSE_ALL_TRACKS / NOTIFY_HUMAN / 重派規則）
由 `/asp-handoff` skill 的 ESCALATION 類型執行；本節為靜態路由表。
...
| 觸發來源 | 升級路由 |
|----------|---------|
| `auto_fix_loop` 振盪偵測 / 級聯偵測 | `escalate(P2)` |
| `auto_fix_loop` 偷渡偵測 | `escalate(P1)` — 偷渡較嚴重 |
| `auto_fix_loop` 重試耗盡 | `escalate(P2)` → Orchestrator 重派 |
| Orchestrator 重派 2 次仍失敗 | `escalate(P1)` |
| 安全審查發現漏洞 / 生產環境事故 | `escalate(P0)` |
| 品質門重試 2 次失敗 / Dev↔QA 迴路模組 3x 失敗 | `escalate(P2)` |
| 並行軌道不可解衝突 / worktree 衝突無法 rebase | `escalate(P1)`（converge.sh 自動寫 escalation log） |
| Tech debt 累積 | `escalate(P3)` |

AI 收到「需要 escalation」訊號時，呼叫 `/asp-handoff`（ESCALATION 類型），由其決策樹處理：
P0 → PAUSE_ALL_TRACKS + NOTIFY_HUMAN；P1 → PAUSE_TRACK + Orchestrator 嘗試解決；
P2 → 嘗試 reassign，不可則升 P1；P3 → LOG_TECH_DEBT。
記錄寫入 `.asp-escalation.ndjson`（audit-write.sh wrapper）。

> 本節隨 global_core 永遠載入；v4 的 `IF escalation_loaded` fallback 分支已無存在理由，
> 各 profile 一律直接走升級路由（ADR-014 D5）。

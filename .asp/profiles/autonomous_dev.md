...
2. 所有待實作功能的 ADR 已為 `Accepted` 或 `FIRM` 狀態（可透過「批次預審」一次完成）
...
| **SPEC 建立** | `make spec-new` + 填寫內容 | 對應 ADR 已 Accepted 或 FIRM（🟡）|
...
| 類別 | 說明 |
|------|------|
| **ADR 狀態變更** | AI 不可自行將 ADR 從 Draft 改為 FIRM 或 Accepted |
...
| 建立/修改 SPEC（ADR 已 Accepted 或 FIRM） | 新增外部依賴（影響所有 Worker） |
